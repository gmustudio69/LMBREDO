...
function s.detachtarget(e,c,tp,r)
	return c:GetOwner()~=e:GetHandlerPlayer() and c:GetPreviousLocation(LOCATION_OVERLAY)~=0 and c:GetReason()&(REASON_COST|REASON_EFFECT)~=0 and and c:GetReasonPlayer(e:GetHandlerPlayer())~=0 and c:GetReasonEffect(e:GetHandler())~=0
end
