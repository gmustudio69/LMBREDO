...
function s.selfilter(c,e,tp,check)
	return c:IsSetCard(0x7da) and c:IsDefense(1800) and c:IsType(TYPE_MONSTER) and (c:IsAbleToHand() or check and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP_DEFENSE))
end

function s.spfilter(c,re,tp)
	return c:IsType(TYPE_MONSTER) and c:IsReason(REASON_COST) and re:IsActivated() and re:IsActiveType(TYPE_MONSTER)
end
