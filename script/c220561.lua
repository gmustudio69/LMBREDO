...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	-- Banish instead of GY (Except Field or Detached)
...
function s.rmtarget(e,c,tp,r)
	return c:GetOwner()~=e:GetHandlerPlayer() and (c:GetPreviousLocation(LOCATION_ONFIELD)==0 or (c:GetPreviousLocation(LOCATION_OVERLAY)~=0 and c:GetReason()&(REASON_COST|REASON_EFFECT)~=0 and c:GetReasonPlayer(e:GetHandlerPlayer())==0 and c:GetReasonEffect(e:GetHandler())~=0))
end
...
function s.copyfilter(c,e,tp,eg,ep,ev,re,r,rp)
	if not (c:IsSetCard(0xf86) and c:IsType(TYPE_TRAP) and c:IsAbleToRemove()) then return false end
...
function s.copyop(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=g:Filter(Card.IsType,nil,TYPE_TRAP):GetFirst()
